<template>
  <div class="detail-page page-shell">
    ...
    <section v-else class="paper-list">
      <article v-for="(paper, idx) in paperList" :key="'paper-' + idx + '-' + paper.title" class="paper-row">
        ...
        <div class="paper-actions">
          <button type="button" class="graph-btn" @click="openGraph(paper)">关系图谱</button>
...
function openGraph(paper: PaperRecord) {
...
.detail-page{max-width:1500px;padding:0 0 40px}.result-header{padding:0 0 20px;border-color:var(--sz-line)}.result-title h2{color:var(--sz-navy);font-size:29px}.ghost-btn,.plain-btn{color:var(--sz-primary)!important;border-color:#cbd6ed;border-radius:8px}.toggle-btn,.search-row button,.load-more button{background:var(--sz-primary);border-radius:8px}.filter-panel{padding:18px;border-color:var(--sz-line);border-radius:var(--sz-radius);box-shadow:var(--sz-shadow)}.paper-list{gap:12px}.paper-row{padding:22px;border-color:var(--sz-line);border-radius:12px;box-shadow:0 4px 18px #1f2f530a}.paper-row:hover{border-color:#b6c4e2}.paper-title-line h3{color:var(--sz-navy);font-size:18px}.paper-title-line span{color:var(--sz-primary)}.paper-actions .graph-btn{background:var(--sz-primary)}.paper-actions button{background:#6f7f97;border-radius:7px}.active-scope span,.meta-line span,.meta-line a,.keywords span{background:var(--sz-primary-faint)}
</style>
