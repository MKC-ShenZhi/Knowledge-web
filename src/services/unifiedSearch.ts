...
const emptyGroups = (): ResourceSearchGroups => ({ paper: [], patent: [], scholar: [], projectFund: [] })
...
function relatedEntries(query: string): Omit<ResourceSearchGroups, 'paper'> {
  return {
    ...
    projectFund: [
      { id: `project-${query}-1`, type: 'projectFund', title: `${query}关键技术与知识服务平台`, summary: `面向科研全过程构建数据治理、语义检索、关系发现和辅助决策能力。`, meta: ['科研项目', '重点研发', '在研', '知识服务'] },
      { id: `project-${query}-2`, type: 'projectFund', title: `多模态${query}分析与可信推理`, summary: `研究多模态信息对齐、可信推理与场景化验证，形成可扩展的技术框架。`, meta: ['科研项目', '联合攻关', '多模态', '可信推理'] },
      { id: `project-${query}-3`, type: 'projectFund', title: `${query}科研数据开放与评价体系`, summary: `建设标准化数据资源，探索面向成果、团队与机构的多维评价方法。`, meta: ['科研项目', '数据开放', '科研评价', '平台建设'] },
      { id: `fund-${query}-1`, type: 'projectFund', title: `${query}基础理论与方法研究`, summary: `支持围绕${query}基础问题、核心算法和验证体系开展系统研究。`, meta: ['科研基金', '国家自然科学基金', '面上项目', '基础研究'] },
      { id: `fund-${query}-2`, type: 'projectFund', title: `面向前沿交叉的${query}创新研究`, summary: `聚焦${query}与生命科学、材料科学及社会科学的交叉创新。`, meta: ['科研基金', '重点项目', '交叉科学', '前沿探索'] },
      { id: `fund-${query}-3`, type: 'projectFund', title: `${query}青年科研人才支持计划`, summary: `支持青年学者围绕${query}开展原创性、探索性研究。`, meta: ['科研基金', '青年项目', '人才支持', '原创研究'] },
    ],
  }
}
